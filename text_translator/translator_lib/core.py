...
def get_translation(text: str, model_name: str, api_base_url: str, glossary_text: Optional[str] = None, debug: bool = False, use_reasoning: bool = False, line_by_line: bool = False) -> str:
    ...
    This function constructs the appropriate prompt, sends it to the API,
    validates the response using `is_translation_valid`, and retries the
    request up to 3 times with exponential backoff if validation fails.
